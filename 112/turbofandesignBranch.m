%PLOTS Contours of constant T04 and Compressor Pressure
...
%         TSFC = f/((1+f)*ue-u)*1000; %units of kg/kNs
        ...
        nth(i,j) = (0.5*(1+f)*ue^2)-(0.5*u^2);
        nth(i,j) = nth(i,j)/(f*Q*1000); %1000 factor for kJ to J

        np(i,j) = ST*u*1000; %multiply by 1000 to restore N
        np(i,j) = np(i,j)/((0.5*(1+f)*ue.^2)-(0.5*u^2));

        no(i,j) = np(i,j)*nth(i,j);
        hold on  
...
fprintf('Plotting...\n',i,length(B))  
figure(2)
contour(pr,tmax,nth,'ShowText','on')
title('Thermal Efficiency')
xlabel('Compressor Ratio')
ylabel('Maximum Combustion Temperature')

figure(3)
contour(pr,tmax,np,'ShowText','on')
title('Propulsive Efficiency')
xlabel('Compressor Ratio')
ylabel('Maximum Combustion Temperature')

figure(4)
contour(pr,tmax,no,'ShowText','on')
