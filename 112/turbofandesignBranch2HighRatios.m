%PLOTS PERFORMANCE WRT Bypass and Fan Pressure Higher Bypass ratio
...
%preallocation 
arrayST = zeros(length(tmax),length(pr));
arrayTSFC = zeros(length(tmax),length(pr));
...
for i=1:length(B)
    for j=1:length(prf)
        ...
        gamma = 1.4;
        P08 = P02*prf(j);
        T08 = T02*(1+(1/etaF)*(prf(j)^((gamma-1)/gamma)-1));
        %fan nozzle exit
        gamma = 1.4;
        uef = sqrt(2*etaFn*(gamma/(gamma-1))*R*T08*(1-(Pa/P08)^((gamma-1)/gamma)));
        %turbine exit
        gamma = 1.33;
        T05 = tmax-(T03-T02)-B(i)*(T08-T02);
        %solve for adiabatic compressor efficency from polytropic
        syms etaAT
        fpt = (T05/tmax)^((gamma*etaPT)/(gamma-1)) == (1+((T05/tmax)-1)*etaAT)^(gamma/(gamma-1));
        ...
        %thrust calculations
        STbare = ((1+f)*ue + B(i)*uef - u*(1+B(i)))/1000; %units of kNs/kg
        Tbare = STbare*mdotA;
        T = Tbare/(1.04+0.01*B(i)^1.2); %thrust in KN
    
%        ST = T/mdotA;
        ST = STbare/(1.04+0.01*B(i)^1.2);
        TSFC = f/ST;
         ST = T/(mdotA*(1+B(i)));


%                 TSFCbare = (f/((1+f)*ue + B(i)*uef- u*(1+B(i))))*1000; %units of kg/kNs
%                 STbare = TSFCbare/f;
%                 Tbare = STbare*mdotA;
%                 T = Tbare/(1.04+0.01*B(i)^1.2);
%                 ST = double(T/((mdotA)*(1+B(i))));
%                 TSFC = double(f/ST);
...
fprintf('\nTransposing...\n',i,length(B))  
arrayST = transpose(arrayST);
arrayTSFC = transpose(arrayTSFC);

fprintf('Plotting...\n',i,length(B))  
contour(B,prf,arrayST,'ShowText','on')
title('Specific Thrust at T04: 1800K and PC:36')
xlabel('Bypass Ratio (B)')
ylabel('Fan Pressure Ratio')

figure(2)
contour(B,prf,arrayTSFC,'ShowText','on')
title('TSFC at T04: 1800K and PC:36')
xlabel('Bypass Ratio (B)')
ylabel('Fan Pressure Ratio')