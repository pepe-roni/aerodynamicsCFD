...
maxpayload = 52000;
diffpayload = maxpayload - 2000*payload;
fuelWeightMaxPayload = W_fuel - diffpayload;
finalWeightMaxPayload = WT_O - fuelWeightMaxPayload;
R_maxPayload = (Vcruise/SFC_Cruise)*L_d*log(WT_O/finalWeightMaxPayload);
R_3 = R;

w04 = WT_O - payload*2000;
w14 = w04 - W_fuel;
R_4 = (Vcruise/SFC_Cruise)*L_d*log(w04/w14);

rangePlot = [0 R_maxPayload R_3 R_4];
payloadPlot = [maxpayload maxpayload payload*2000 0];
rangePayloadPlot = plot(rangePlot,payloadPlot);
xlabel('Range (nmi)');
ylabel('Payload (lbs)');
title('Payload-Range Plot - Composite');
...
fprintf('Thrust Required Per Engine: %.1f lbs\n',Te)
fprintf('Wing Area: %.2f sqft\n', S)
fprintf('Length of Fuselage: %.2f ft\n', l)
fprintf('Diameter of Fuselage: %.2f ft\n', d)
fprintf('t/c: %.5f\n', tc)
fprintf('Span: %.3f ft\n\n', b)
...
fprintf('Combined Drag Coefficient: %.5f\n', C_d)
...
fprintf('Utilization: %.2f hr/yr\n', U)
fprintf('Maintenance Costs: %.5f $/ton mi\n',Ctm_maint)
fprintf('Total Airplane Cost: $%.5f \n',Ct)
fprintf('Direct Operating Costs: %.5f $/ton mi\n',DOC)
fprintf('Direct Operating Costs: %.5f $/PAX\n', PAXDOC)
fprintf('Inflation Adjusted Airplane Cost: $%.5f \n',Ct*5.34)
fprintf('Inflation Adjusted Maintenance Costs: %.5f $/ton mi\n',Ctm_maint*5.34)
fprintf('Inflation Adjusted DOC: %.5f $/ton mi\n',DOC*5.34)
fprintf('Inflation Adjusted DOC: %.5f $/PAX mi\n',PAXDOC*5.34)
